...
-- Creates the staff_invites table and related functions for managing staff invitations

-- 1. Ensure users table has status metadata (if not already exists)
...
-- 3. Create update_updated_at_column function if not exists
...
-- 4. Create trigger for staff_invites updated_at
...
-- Allow school administrators to view invites
...
-- Allow school administrators to create invites
...
-- Allow school administrators to update invites
...
-- 7. Helper function for invite validation (used during signup)
...
-- 8. Helper function for consuming invites (marking as accepted)
